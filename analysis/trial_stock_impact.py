...
def fetch_trial_details(nct_id: str) -> dict:
    ...
    print(f'[ctgov] Phase: {phase_str}, Enrollment: {enrollment.get("count")}, Has Results: {data.get("hasResults")}')
    return details
...
def fetch_ohlc(ticker: str, from_date: str, to_date: str) -> pl.DataFrame:
    ...
    df = df.with_columns(
        pl.from_epoch('t', time_unit='ms').alias('date')
    )
...
def calculate_returns(df: pl.DataFrame, event_date: str) -> pl.DataFrame:
    ...
    df = df.with_columns([
        ((pl.col('c') - pl.col('c').shift(1)) / pl.col('c').shift(1) * 100)
...
def main(sponsor_filter: str | None = None):
    ...
    if not pre.is_empty() and not post.is_empty():
        ...
        print(f"Event day close: ${ohlc.filter(pl.col('date').cast(pl.Date) >= datetime.strptime(completion_date, '%Y-%m-%d').date()).head(1)['c'][0]:.2f}")
